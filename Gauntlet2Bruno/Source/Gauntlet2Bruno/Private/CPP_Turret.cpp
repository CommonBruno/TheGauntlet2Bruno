	...
	if (Player)
	{
		...
		if (bCanSee)
		{
			...
			if (FireCooldown >= (1 / FireRate))
			{
				...
				FVector LocalFirePoint = FirePoint.GetLocation();
				FVector CalculatedStartPoint = GetActorTransform().TransformPosition(LocalFirePoint);
				
				Data.ProjectileDir = Player->GetActorLocation() - CalculatedStartPoint;
